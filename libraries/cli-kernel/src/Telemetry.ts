...
const endpoint = Env.optional('OTEL_EXPORTER_OTLP_TRACES_ENDPOINT');
...
export const layer = (
  service: Service,
): Layer.Layer<never, never, FileSystem.FileSystem | Path.Path | ConfigDirectory> =>
  Layer.unwrap(
    Effect.gen(function* () {
      ...
      // Telemetry must never be why the tool fails. `Consent.recorded` already
      // absorbs a damaged decision file; this covers what is left, an endpoint
      // that will not parse, on the same principle.
...
/**
 * Traces `effect` as a named span without ever handing its failure to the
 * tracer.
 *
 ...
 *
 * The annotating is a finalizer rather than a step after the `exit`, because a
 * fiber interrupted from outside does not run its continuations — it unwinds.
 * Ctrl-C is the one outcome a CLI can most expect to see, and as a `tap` the
 * annotation was silently dropped for exactly that case, leaving a span with
 * nothing on it and making the `Interrupted` tag above unreachable. Marking the
 * `tap` uninterruptible does not help: the problem is not that the step is cut
 * short, it is that it is never reached. Finalizers are.
 *
 * The `Exit` a finalizer sees is the outer one: a `Success` carrying the inner
 * `Exit` when the effect ran to a conclusion of its own, and a `Failure` only
 * when the fiber itself was interrupted. Both are worth a tag. A span that ends
 * interrupted is safe to report: `OtlpTracer` marks it `status.interrupted` and
 * attaches no exception, so there is still no message and no stack trace.
 */
const traced =
