 ...
 *
 * `decidedAt` is an instant, not a string that happens to look like one. The
 * schema owns the encoding in both directions, so a file whose timestamp is
 * unparseable is rejected on read rather than carried around and printed back
 * at the user as though it meant something.
 */
...
/**
 * Writes the decision so that a run cut short leaves either the old one or the
 * new one, and never half of either.
 *
 * Written beside the target and renamed over it: `rename` within a directory is
 * atomic, and a neighbour is guaranteed to be on the same filesystem, which a
 * system temp directory is not. Writing in place would be the more obvious
 * thing and the wrong one — {@link stored} refuses to guess at a file it cannot
 * parse, so a torn write does not degrade to re-asking the question, it wedges
 * every later run until someone finds the file and deletes it.
 *
 * The layout is `JSON.stringify`'s, but the field encoding is the schema's:
 * this is a file a user is invited to open, so it is worth indenting, and that
 * is a presentational choice rather than licence to hand-roll the contents.
 */
const persist = (decision: Decision) =>
  Effect.gen(function* () {
    ...
    yield* fileSystem.writeFileString(temporary, `${JSON.stringify(encoded, null, 2)}\n`).pipe(
      ...
      // A failed write must not leave its scratch file behind, and a failure to
      // clean up is not worth reporting over whatever actually went wrong.
 ...
 *
 * The way back out is the file, and the command only where there is one to
 * name. A binary run straight from a registry is never on the user's path, so
 * the name it answers to is not something that user can type, while the file is
 * an instruction that holds however the binary was reached.
 */
 ...
 * `Telemetry.layer` chose a tracer. Consent given here covers the next run.
 *
