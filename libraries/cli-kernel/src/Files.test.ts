...
describe('apply, into a root that does not exist', () => {
  ...
  it('leaves no root at all when a write fails partway', async () => {
    // The second entry names a path the first has already made a directory, so
    // its exclusive create fails after some of the tree is on disk.
...
describe('apply, into a root that already exists', () => {
  ...
  it('undoes everything it did when a write fails partway', async () => {
    ...
    // The created file, the directory created for it, and every backup and
    // scratch file taken along the way are all gone.
