import { Ask, Consent, Files, Telemetry, Ui } from '@falkara/cli-kernel';
...
// The first entry is the default: it is what "(recommended)" points at, what
// `--yes` picks, and what the select highlights when it opens.
...
const defaultPackageManager: PackageManager = packageManagers[0].value;
...
/**
 * Not an answer to be prompted for, and so not part of {@link Answers}: it says
 * what to do with the answers rather than supplying one. A run that asked "did
 * you mean it?" would defeat the point of a flag that exists to be deliberate.
 */
...
/**
 * What the invocation said, before anything has been asked.
 *
 * Named rather than inferred so that {@link resolveAnswers} can be written and
 * read as a function of it, instead of as a closure over whatever
 * `Command.make` happened to hand the handler.
 */
interface Inputs {
...
/**
 * Settles every question the scaffolding needs answered.
 *
 * Separate from {@link execute} because the two fail for unrelated reasons and
 * are interesting to a reader at different times: this half is where the flags,
 * the defaults and the prompts are reconciled and where a run can turn out to
 * be under-specified, and it does nothing to the filesystem while deciding.
 */
const resolveAnswers = Effect.fnUntraced(function* (inputs: Inputs) {
  // `--yes` is fixed for the whole run, so it is bound once here.
  const answer = Ask.asking(inputs.yes);
...
/**
 * Describes the run on the current span.
 *
 * Only the shape of it: which package manager, whether dependencies were
 * installed, whether anyone was asked. The name and directory the user chose
 * are theirs and are deliberately absent. What machine it ran on is added by
 * `Telemetry.operation`, which every command goes through.
 */
...
/**
 * Does the work the answers describe.
 *
 * Separate from {@link resolveAnswers} because this is the half that touches
 * the disk: it takes a settled `Answers` and asks nothing, which is what makes
 * it runnable from a test without a terminal anywhere in sight.
 *
 * The plan is built before the branch, so `--dry-run` and a real run survey the
 * same set of files. A preview that reported what a run would do, rather than
 * what this run is about to do, would be worth nothing.
 *
 * Measured under a span of its own, so that the duration on record is this and
 * not the banner, the consent notice and however long someone took over four
 * prompts.
 */
const execute = Effect.fnUntraced(function* (answers: Answers, handling: Handling) {
  ...
  // Dies rather than fails: a template naming a path outside its own root, or
  // two naming the same one, is a bug in Falkara. Threading it through the
  // error channel would ask every caller to handle something no user can act on.
  ...
  if (answers.install) {
    // TODO: run the package manager. Reported rather than drawn as a completed
    // step, because a tick against work that did not happen is worse than no
    // step at all.
    yield* ui.report({
      severity: 'warning',
...
}, Telemetry.measured('workspace.create.scaffold'));

/**
 * Announces the tool.
 *
 * Outside the traced operation, and deliberately: it is a second of deliberate
 * pacing that would otherwise be the largest term in every duration the run
 * reports.
 */
...
export class Consenting extends Context.Service<
  Consenting,
  {
    ...
    /** Absent in a binary a user cannot invoke by name, where the notice names the decision file alone. */
...
const run = Effect.fnUntraced(
  function* (inputs: Inputs) {
    ...
    // Asked after the banner rather than at startup, so the user has seen what
    // this tool is before being asked to share anything about using it.
    ...
    // Last of the questions and before the first write, so a run that answers
    // it has still answered nothing about the workspace it is about to create.
  ...
  // The span is applied here rather than by `Effect.fn`, because a span that
  // sees a failure exports its message and stack trace, and both are full of
 ...
 */
export const flags = { name, directory, packageManager, install, yes, force, dryRun };

/** What the command says it is for, wherever it is mounted. */
export const description =
  'Scaffold a new Falkara Workspace project. Any answer not supplied as a flag is ' +
