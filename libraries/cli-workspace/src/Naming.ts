...
// Characters no portable path may contain. `<>:"|?*` are rejected outright by Windows filesystems and control characters are illegal everywhere, so they are refused regardless of host: a workspace created on macOS should still be checkoutable on Windows.
// eslint-disable-next-line no-control-regex -- rejecting control characters is the intent
const pathHostile = /[<>:"|?*\u0000-\u001F]/u;

// A name needs at least one letter or number in any script: `Проект` is as legitimate a name as `Project`. Whether the slug keeps anything of it is `toDirectory`'s problem, which has a fallback for exactly that.
...
/**
 * Accepts a workspace name, or says why it cannot be one.
 *
 ...
 *
 * @param value What the user offered as a name.
 */
...
// A drive prefix such as `C:\` or `d:/` is the one place a colon is legal in a Windows path, so it is peeled off before the hostile-character check. A bare `C:` is not matched: drive-relative paths resolve against a per-drive working directory, which is never what a scaffold should target.
const drivePrefix = /^[a-zA-Z]:(?=[\\/])/u;

/**
 * Accepts a destination directory, or says why it cannot be one.
 *
 * Separators stay legal here — a directory is allowed to be a path.
 *
 * @param value What the user offered as a directory.
 */
export const validateDirectory = (value: string): Validated => {
  ...
  // Trailing whitespace is already trimmed; Windows also strips a trailing dot, which would silently yield a different directory than the one asked for.
  if (trimmed.endsWith('.')) {
 ...
 *
 * @param validate The rule to apply to what was typed.
 */
 ...
 *
 * @param flag The flag's name, as the refusal quotes it.
 * @param validate The rule to apply to what was passed.
 */
 ...
 *
 * @param name The display name to reduce.
 */
 ...
 *
 * @param name The display name the directory is derived from.
 */
