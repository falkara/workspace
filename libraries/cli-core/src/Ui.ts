...
export const Verbosity = Context.Reference<Verbosity>('@falkara/cli-core/Ui/Verbosity', {
...
/**
 * A run of text and how much it stands out.
 */
...
/**
 * A segment carrying no emphasis.
 *
...
/**
 * A segment the reader's eye should land on.
 *
 * @param text The text to carry.
 */
...
/**
 * A segment that recedes behind the rest of its line.
 *
 * @param text The text to carry.
 */
...
/**
 * A segment marked as the tool's own voice.
 *
 * @param text The text to carry.
 */
...
// Whether a diagnostic of this severity survives this verbosity. A quiet run is quiet about progress, not about problems: warnings and errors are why someone reads the output at all, so they are never what `--quiet` removes.
...
// The styling this presentation may use. Two implementations rather than a flag checked at each call site, for the same reason the service says nothing about colour.
interface Palette {
...
// How a step ended, read from the `Exit` rather than assumed, so a step that failed is never reported with a tick.
...
// Splits a line into one segment per grapheme, so that revealing it paces what the reader sees rather than how the string happens to be encoded.
...
// Builds a `Ui` that writes for a person over a `Terminal`. Every variant announces a step when it starts and marks how it ended; they differ in whether the running indicator is redrawn in place or left standing as a line a log can keep.
    ...
    step: (label, work) =>
      Effect.flatMap(Verbosity, (verbosity) => {
        ...
        return animate
          ? Effect.gen(function* () {
              let frame = 0;
              // Suspended so every repeat advances the frame; building the string eagerly would redraw a single frame forever.
              ...
              // `onExit` rather than `ensuring`: the outcome decides the mark, so a step that failed cannot report a tick. The spinner is stopped before that line is drawn, so no pending frame overwrites it.
          ...
          : // The label lands before the work runs, so a slow step is attributable in a log while it is still going — not only once it has already finished or timed out.
            Effect.andThen(
 ...
 *
 * @param presentation How output is to be delivered.
 */
