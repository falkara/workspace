import { Crypto, Effect, Exit, FileSystem, Path, PlatformError, Runtime, Schema } from 'effect';
...
export interface File {
  ...
  /**
   * Permission bits, where a file needs them.
   */
 ...
 */
export class Escapes extends Schema.TaggedError<Escapes>('@falkara/cli-core/Files/Escapes')(
  'Escapes',
  {
    path: Schema.String,
  },
 ...
 */
export class Collides extends Schema.TaggedError<Collides>('@falkara/cli-core/Files/Collides')(
  'Collides',
  {
    path: Schema.String,
    /**
...
export class Occupied
  extends Schema.TaggedError<Occupied>('@falkara/cli-core/Files/Occupied')('Occupied', {
    path: Schema.String,
    root: Schema.String,
  })
  implements Guidance.Guiding
{
  ...
  /**
   * What the user has to change, phrased for the destination that was taken.
   */
  get guidance(): string {
    return (
 ...
 *
 * @param root The directory every write lands beneath.
 * @param files What the command intends to write.
 */
export const plan = (
  root: string,
...
/**
 * One target, and what committing the plan will do to it.
 */
 ...
 * here is why".
 *
 * @param plan The writes to measure against the disk.
 */
export const survey = (
  plan: Plan,
  ...
  Effect.gen(function* () {
    ...
    for (const target of plan.targets) {
      // A root that does not exist cannot contain anything, so the question is settled for the whole plan rather than asked once per file.
 ...
 * list for.
 *
 * @param surveyed The measured plan to describe.
 */
...
// Commits a plan into a root that does not exist yet. The whole tree is built beside the destination and moved into place with one `rename`, which is atomic and which a directory-at-a-time write is not: an interrupted scaffold leaves no directory at all rather than a half-populated one that the next run then has to reason about.
// The staging directory is a sibling because that is what guarantees the same filesystem, and therefore that the rename is a rename rather than a copy that can fail halfway.
// Every entry is a `create` here by construction — `survey` cannot have found anything to conflict with inside a root that does not exist.
const intoFreshRoot = Effect.fnUntraced(function* (surveyed: Survey) {
  ...
  // The parent chain is created first and deliberately left behind on failure: an empty directory is not a corrupted workspace, and removing one this run may not have been the only thing to create is worse than leaving it.
  ...
  yield* Effect.onExit(
    Effect.gen(function* () {
      ...
      for (const { target } of surveyed.planned) {
        ...
        // `wx` rather than `w`: staging is freshly made, so a destination that already exists means two targets resolved to one path, and failing is better than letting the second silently win.
...
// What has to be undone, and how, if a commit into an existing root fails partway.
type Undo =
...
// Commits a plan into a root that already exists. This is recoverable rather than atomic, and the difference is worth stating plainly: POSIX offers an atomic rename of one path, and nothing that makes many paths change together.
// So each write is journalled and undone in reverse on failure — created files removed, replaced files moved back, directories this run brought into existence taken away again. A process killed outright mid-commit leaves the journal unplayed and the tree partly written; only a failure or an interrupt this process survives is recovered from.
// The alternative, staging the merged tree and renaming it over the root, is atomic and wrong: it would take everything already in the root that the plan says nothing about with it.
const intoExistingRoot = Effect.fnUntraced(function* (surveyed: Survey) {
  ...
  // Ancestors are walked one at a time rather than left to `makeDirectory({ recursive: true })` so the journal records exactly the directories this run brought into existence, and a rollback removes those and nothing that was already there.
  ...
  const commit = Effect.gen(function* () {
    for (const { target, disposition } of surveyed.planned) {
      ...
      if (disposition === 'create') {
        // `wx` fails if anything appeared between the survey and now, so the promise that nothing existing was touched is kept by the kernel rather than by the gap between a check and a write.
      ...
      // Written first, moved aside second, swapped in third. Ordered that way so a write that fails — the overwhelmingly likely failure, being the only step that touches the disk in bulk — has disturbed nothing at all.
  ...
  // Both are suspended so the journal is read when they run rather than when they are built — by construction time `commit` has not recorded anything yet, and an undo list snapshotted then is empty for ever.
      ...
      (undo) => {
        switch (undo._tag) {
          ...
          case 'RemoveDirectory':
            // Emptiness is checked rather than left to `remove` to enforce: a non-recursive remove refuses every directory, empty or not, so relying on it to fail would quietly undo no directory at all. A directory that is no longer empty holds something this run did not put there, and is left exactly as it is.
  ...
  // A finalizer rather than a step after the commit: a fiber interrupted from outside unwinds rather than continuing, and an interrupted half-written tree is exactly what this exists to clean up.
 ...
 * destination allows.
 *
 * The plan is built as a value, surveyed against the disk, and only then
 * committed. That order is what makes `--dry-run` honest: a preview and a real
 * run are the same {@link plan} and the same {@link survey}, so a preview that
 * promises twelve files and a run that then refuses on the thirteenth cannot
 * happen. A boolean threaded down to each write would be the obvious shape and
 * the wrong one — it puts the branch next to the `writeFileString`, which is
 * precisely where someone eventually forgets it.
 *
 ...
 *
 * @param plan The writes to commit.
 */
export const apply = (
  plan: Plan,
): Effect.Effect<
