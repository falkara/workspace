 ...
 *
 * @param permissions What the terminal is to be treated as permitting.
 */
export const layerOf = (permissions: Permissions): Layer.Layer<Capabilities> =>
  Layer.succeed(Capabilities)(permissions);

// Whether the caller has asked for colour regardless of where output lands. `FORCE_COLOR=0` is how the same convention says no, and is treated as not forcing rather than as forcing nothing.
...
// `NO_COLOR` beats `FORCE_COLOR`: the two conventions do not say which wins, so the tie goes to the one asking for less. Someone who exported `NO_COLOR` globally and `FORCE_COLOR` for one tool years ago should not be surprised by escape bytes.
// `TERM=dumb` disables all three, not merely colour. Cursor movement is what the animated presentation is built out of, and a prompt cannot redraw itself on a terminal that cannot move the cursor either.
