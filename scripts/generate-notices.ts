...
const manifestPath = path.join(applicationDirectory, 'package.json');
// Taken from `bin` rather than assumed: the manifest is what a package manager installs, so it is the only entry that cannot disagree with what was published.
const installed: string = Object.values(
  JSON.parse(fs.readFileSync(manifestPath, 'utf8')).bin as Record<string, string>,
)[0] as string;
const bundlePath = path.resolve(applicationDirectory, installed);
